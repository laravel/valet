...
class BrewTest extends PHPUnit_Framework_TestCase
{
    ...
    public function test_brew_is_available()
    {
        $cli = Mockery::mock(CommandLine::class);
        $cli->shouldReceive('run')->once()->with('which brew', Mockery::type('Closure'))->andReturn('/usr/bin/apt');
        swap(CommandLine::class, $cli);
        $this->assertTrue(resolve(PackageManager::class)->isAvailable());
    }


    public function test_brew_is_not_available()
    {
        $cli = Mockery::mock(CommandLine::class);
        $cli->shouldReceive('run')->once()->with('which brew', Mockery::type('Closure'))->andReturn('');
        swap(CommandLine::class, $cli);
        $this->assertFalse(resolve(PackageManager::class)->isAvailable());

        $cli = Mockery::mock(CommandLine::class);
        $cli->shouldReceive('run')->once()->with('which brew', Mockery::type('Closure'))
