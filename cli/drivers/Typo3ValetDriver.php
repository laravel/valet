...
class Typo3ValetDriver extends ValetDriver
{
    ...
    public function serves($sitePath, $siteName, $uri)
    {
        ...
        return file_exists($typo3Dir) === true && is_dir($typo3Dir) === true;
    }
    ...
    public function isStaticFile($sitePath, $siteName, $uri)
    {
        ...
        if ($this->isActualFile($filePath = $sitePath . $this->documentRoot . $uri) === false) {
        ...
        if ($this->isActualFile($filePath = $sitePath . $this->documentRoot . $uri) === true) {
    ...
    private function isAccessAuthorized($uri)
    {
        foreach ($this->forbiddenUriPatterns as $forbiddenUriPattern) {
            if (preg_match("@$forbiddenUriPattern@", $uri) === 1) {
                return false;
            }
        }
        return true;
    ...
    public function frontControllerPath($sitePath, $siteName, $uri)
    {
        ...
        if (file_exists($absoluteFilePath = $sitePath . $this->documentRoot . $uri) === true) {
            if (is_dir($absoluteFilePath) === true) {
                if (file_exists($absoluteFilePath . '/index.php') === true) {
                    // this folder can be served by index.php
                    return $this->serveScript($sitePath, $siteName, $uri . '/index.php');
                }

                if (file_exists($absoluteFilePath . '/index.html') === true) {
